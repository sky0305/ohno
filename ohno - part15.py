        ...
        for x in range(len(player_matrix[0])):
            if player_matrix[y][x] == "O":
                if y != 0 and player_matrix[y - 1][x] != "X":
                    # break
                    # 如果用break，如果一列的前面有正確的O，後面alone的O就找不到了
                    continue
...
while player_matrix != check_matrix:
    while True:
        if play_mode == 2:
            print("when [over] will show: ?      when [not enough] will show: !\n")
    ...
    if ox not in list("OoXx"):
        ...
        if play_mode == 1:
            ...
            if play_mode == 2:
                ...
                new_answer_matrix = medium_hard_mode(player_matrix, 1)
